# note-of-C-plus-plus-primer

再刷一遍primer，记些笔记，查漏补缺

**二、基本数据类型**

1. int 最小是16位的，意味着int在不同的编译器中即有可能是16位也有可能是32位
...
3. 默认情况下，打开一个ofstream的时候，会把里面原来的文件内容全部丢弃掉，贼坑！！！！解决方法是加一个app模式，例如
...
11. 注意：vector<bool>并不是一个容器，虽然他在形式上看起来像是一个容器，但是实际上他是按照位来进行存储的（为了节省空间），但是相对应的，他的时间效率要比其他的vector慢40倍以上（可能是因为底层的非连续存储）并且在引用的时候会出现非常奇怪的问题，尽量避免使用
        
12. 对于vector<vector<T>>在使用for的时候，一定要加&引号，例如下面：
        
        vector<vector<int>> v = {{1,0}, {-1,0}, {0,-1}, {0,1}};
        for(auto a : v){}
        for(auto& a : v){}
        
上面的代码就比下面的代码慢无数倍，因为auto的时候每次都要重新构造一遍vector，非常的慢
...
除了初始化以外，使用方法和普通指针一样：
        ...
        shared_ptr<string> p1; *p1 = "hi"; swap(p ,q)//交换pq的指针。推荐使用make_shared来进行shared_ptr的创建，相对比较安全（能够防止例如将同一块内存绑定到多个shared_ptr上面：
        shared_ptr<int> p3 = make_shared<int>(42);

3. shared_ptr 有一个问题，就是在最后一个对象销毁前都不会释放内存，所以如果把shared_ptr放在容器中重拍后不需要全部元素的时候，要记得erase那些不需要的元素。
4. 使用auto进行变量的初始化（不建议使用）：
